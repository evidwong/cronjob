...
class Wechat
{
	...
	const MSGTYPE_DEVICE = 'device_text';
	...
	const CON_MENU_DELETE_URL = '/menu/delconditional?';
	...
	const TPL_ALL = '/template/get_all_private_template?';
	const TPL_DEL = '/template/del_private_template?';
	///数据分析接口
	...
	//设备接入
	const DEVICE_GETDIVICEID = '/device/getqrcode?'; //蓝牙设备二维码获取
	const DEVICE_AUTHORIZE = '/device/authorize_device?'; //蓝牙设备授权
	const DEVICE_BIND = '/device/bind?'; //绑定设备
	const DEVICE_VERIFY_QRCODE = '/device/verify_qrcode?'; //验证设备二维码
	const DEVICE_GET_STAT = '/device/get_stat?'; //设备状态查询
	const DEVICE_TRANSMSG = '/device/transmsg?'; //发送信息到设备
	const DEVICE_GET_OPENID = '/device/get_openid?'; //获取绑定的openid
	const DEVICE_GET_USER_BIND_DEVICE = '/device/get_bind_device?'; //获取用户绑定的设备
	const DEVICE_UNBIND = '/device/unbind?'; //设备解绑
	const DEVICE_CUNBIND = '/device/compel_unbind?';
	const DEVICE_COMPELBIND = '/device/compel_bind?';

	const MEMBER_TAG_SET = '/tags/members/batchtagging?';
	const MEMBER_TAG_DEL = '/tags/members/batchuntagging?';
	const USER_TAG_GET = '/tags/get?';
	const USER_TAG_CREATE = '/tags/create?';
	const USER_TAG_EDIT = '/tags/update?';
	const USER_TAG_DEL = '/tags/delete?';
	const USER_TAG_FANS_GET = '/user/tag/get?';

	const CREATE_TAG_MENU = '/menu/addconditional?';
	// const QUERY_TAG_MENU='';
	...
	{
		$this->redis = app('redis.companyInfo');
		if (!$this->redis) return false;
		$this->_conf = $this->redis->hGetAll('wechat_config:' . $this->cid);
	...
	{
		if (!$cid) return false;
		$setConfig = self::_init();
		if (!$setConfig) return false;
		$this->token = $this->_conf['token'];
		$this->encodingAesKey = $this->_conf['encodingaeskey'];
		$this->appid = $this->_conf['appid'];
		$this->appsecret = $this->_conf['appsecret'];
		$this->debug = $this->_conf['debug'];
		$this->logcallback = $this->_conf['logcallback'];
	}
	...
	private function checkSignature($str = '')
	{
		...
		$tmpStr = sha1($tmpStr);
		// file_put_contents('./tmpstr_sign.log', var_export($tmpStr.'------'.$signature,true),FILE_APPEND);
		if ($tmpStr == $signature) {
			return true;
		} else {
			return false;
	...
	public function valid($return = false)
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == "POST") {
			...
			if ($this->encrypt_type == 'aes') { //aes加密
				...
				$pc = new \Lib\Prpcrypt($this->encodingAesKey);
			...
			if ($return) {
				...
				file_put_contents('./checksignature.log', var_export($this->checkSignature(), true), FILE_APPEND);
				if ($this->checkSignature())
					die($echoStr);
				else
					die('no access');
	...
	public function news2($newsData = array())
	{
		$this->Message($newsData);
		return $this;
	}
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	/* protected function setCache($cachename,$value,$expired){
		$_SESSION['access_token'][$cachename]=$value;
		$_SESSION['access_token']['expired']=$expired;
		$_SESSION['access_token']['start_time']=time();
		if ($_SESSION['access_token']) {
			return true;
		}
		return false;
	} */
	protected function setCache($json, $type = 'access_token')
	{
		global $_G;
		$time = time() - 10 + $json['expires_in'];
		if ($type == 'access_token') {
			$token = $json['access_token'];
			$_G['wechat_configs']['access_token'] = $token;
			$_G['wechat_configs']['token_time'] = $time;
			$_G['wechat_configs']['access_token_expired'] = $time;
		} elseif ($type == 'jsapi_ticket') {
			$token = $json['ticket'];
			$_G['wechat_configs']['jsapi_ticket'] = $token;
			$_G['wechat_configs']['ticket_time'] = $time;
			$_G['wechat_configs']['jsapi_ticket_expired'] = $time;
		}
		$this->redis->setex($type . ':' . $_G['company_id'], $json['expires_in'] - 50, $token);
			// 如果redis主服务器正常，写/更新redis
		$this->redis->hMset('wechat_config:' . $_G['company_id'], array($type => $token, $type . '_expired' => $time));
		// 查找数据库，如果有，更新数据库，没有则写数据库
		$row = M('config')->where(array('cid' => $_G['company_id'], 'keyname' => $type))->find();
		if ($row) {
			// 数据库有记录
			$result = M('config')->where(array('cid' => $_G['company_id'], 'keyname' => $type))->save(array('keyvalue' => $token, 'addtime' => time(), 'updatetime' => $time));
		} else {
			// 数据库没有记录
			$result = M('config')->add(array('cid' => $_G['company_id'], 'keyname' => $type, 'keyvalue' => $token, 'addtime' => time(), 'updatetime' => $time));
		}
		if ($result !== false) {
			// 数据库更新成功，设置全局 $_G，返回TRUE
			return true;
		} else {
			return false;
		}
	...
	{
		global $_G;
		if ($_G['wechat_configs'][$type . '_expired'] <= time()) {
			return false;
		}
		return $_G['wechat_configs'][$type];
	}
	/* protected function getCache($cachename){
		if ($_SESSION['access_token'] && ((time()-$_SESSION['access_token']['start_time'])<($_SESSION['access_token']['expired']-100))) {
// 			var_dump($_SESSION);
			return $_SESSION['access_token'][$cachename];
		}else{
	...
	protected function removeCache($type = 'access_token')
	{
		global $_G;
		$this->redis->delete($type . ':' . $_G['company_id']);
		$this->redis->hDel('wechat_config:' . $_G['appid'], $type);
		$this->redis->hDel('wechat_config:' . $_G['appid'], $type . '_expired');
		$row = M('config')->where(array('cid' => $_G['company_id'], 'keyname' => $type))->find();
		if ($row) {
			M('config')->where(array('cid' => $_G['company_id'], 'keyname' => $type))->save(array('updatetime' => 0));
		}
		return true;
	}
	/**
	...
	public function checkAuth($appid = '', $appsecret = '', $token = '')
	{
		...
		$rs = $this->getCache('access_token');
		if ($rs) {
			// var_dump($rs);
			$this->access_token = $rs;
			return $rs;
		}
		$result = $this->http_get(self::API_URL_PREFIX . self::AUTH_URL . 'appid=' . $_G['wechat_configs']['appid'] . '&secret=' . $_G['wechat_configs']['appsecret']);
		// var_dump($result);
		if ($result) {
			...
			// M('config')->where(array('cid'=>$_G['company_id'],'keyname'=>'access_token'))->save(array('access_token'=>$json['access_token'],'updatetime'=>time()))
			$this->access_token = $json['access_token'];
			$setToken = $this->setCache($json, 'access_token');
			if ($setToken) {
				return $this->access_token;
			}
		}
	...
	public function getJsTicket()
	{
		...
		if ($result) {
			...
			$setToken = $this->setCache($json, 'jsapi_ticket');
			if ($setToken) {
				return $this->jsapi_ticket;
			}
			return $this->jsapi_ticket;
	...
	{
		file_put_contents('./getJsSign.log', "[" . date('Y-m-d H:i:s') . "] " . (isset($_SESSION['member']['nickname']) ? $_SESSION['member']['nickname'] : '') . " start \n", FILE_APPEND);
		if (!$this->jsapi_ticket && !$this->getJsTicket($appid) || !$url) return false;
		if (!$timestamp)
			$timestamp = time();
		if (!$noncestr)
			$noncestr = $this->generateNonceStr();
		file_put_contents('./getJsSign.log', "[" . date('Y-m-d H:i:s') . "] " . (isset($_SESSION['member']['nickname']) ? $_SESSION['member']['nickname'] : '') . " generateNonceStr \n", FILE_APPEND);
		$ret = strpos($url, '#');
		if ($ret)
		...
		file_put_contents('./getJsSign.log', "[" . date('Y-m-d H:i:s') . "] " . (isset($_SESSION['member']['nickname']) ? $_SESSION['member']['nickname'] : '') . " sign \n", FILE_APPEND);
		if (!$sign)
			return false;
		$signPackage = array(
			"appid"     => $this->appid,
		...
		file_put_contents('./getJsSign.log', "[" . date('Y-m-d H:i:s') . "] " . (isset($_SESSION['member']['nickname']) ? $_SESSION['member']['nickname'] : '') . " end \n", FILE_APPEND);
		return $signPackage;
	}
	...
	public function deleteConditionalMenu($menuid)
	{
		...
		$result = $this->http_post(self::API_URL_PREFIX . self::CON_MENU_DELETE_URL . 'access_token=' . $this->access_token, json_encode($data));
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function getQRCode($scene_id, $type = 0, $expire = 1800)
	{
		...
		file_put_contents('./qrcode.log', var_export($data, true), FILE_APPEND);
		if ($type == 1) {
	...
	{
		// file_put_contents('openid', $openid);
		if (!$this->access_token && !$this->checkAuth()) return false;
		/*{
			file_put_contents('access_token.log', var_export($this->access_token,true).var_export($this->checkAuth(),true),FILE_APPEND);

		} */
		$result = $this->http_get(self::API_URL_PREFIX . self::USER_INFO_URL . 'access_token=' . $this->access_token . '&openid=' . $openid);
		// file_put_contents('./openid.log', var_export($result,true),FILE_APPEND);
		// var_dump($result);
		if ($result) {
			$json = json_decode($result, true);
			if (isset($json['errcode'])) {
	...
	public function getUserBatchInfo($openid)
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::USER_BATCH_INFO_URL . 'access_token=' . $this->access_token, json_encode($openid));
		// var_dump($result);
		if ($result) {
			$json = json_decode($result, true);
			if (isset($json['errcode'])) {
	...
	{
		file_put_contents('sendCustomMessage.txt', var_export($data, true));
		if (!$this->access_token && !$this->checkAuth()) return false;
	...
	public function sendCustomNewsMessage($data)
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::CUSTOM_SEND_URL . 'access_token=' . $this->access_token, self::json_encode($data));
		file_put_contents('sendCustomNewsMessage.log', var_export($result, true), FILE_APPEND);
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function getTplList($data)
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::TPL_ALL . 'access_token=' . $this->access_token, self::json_encode($data));
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function delTmpl($tplid)
	{
		$data['template_id'] = $tplid;
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::TPL_DEL . 'access_token=' . $this->access_token, self::json_encode($data));
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function sendTemplateMessage($data)
	{
		...
		// file_put_contents('./sendResult.log', var_export($result,true),FILE_APPEND);
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function getDeviceId()
	{
		if (!$this->access_token && !$this->checkAuth())
			return false;
		$result = $this->http_get(self::API_BASE_URL_PREFIX . self::DEVICE_GETDIVICEID . 'access_token=' . $this->access_token);
		$this->log($result);
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
	 ...
	 * );
	 */
	public function getDeviceAuthorize($data)
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_BASE_URL_PREFIX . self::DEVICE_AUTHORIZE . 'access_token=' . $this->access_token, self::json_encode($data));
		$this->log($result);
		if ($result) {
			$json = json_decode($result, true);
	 ...
	 * @return boolean Array
	 */
	public function deviceBind($data)
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
	 ...
	 * 第三方确认用户和设备的解绑操作。
	 * @param unknown $data
	 * @return boolean|mixed|Ambigous <string, boolean, mixed>
	 */
	public function deviceUnbind($data)
	{
		if (!$this->access_token && !$this->checkAuth())
			return false;
		$data = array(
	 ...
	 * @return boolean|mixed|Ambigous <string, boolean, mixed>
	 */
	public function deviceCompelunBind($data)
	{
		if (!$this->access_token && !$this->checkAuth())
			return false;
		$data = array(
		...
		$result = $this->http_post(self::API_BASE_URL_PREFIX . self::DEVICE_CUNBIND . 'access_token=' . $this->access_token, self::json_encode($data));
		$this->log($result);
		if ($result) {
			$json = json_decode($result, true);
			return $json;
	 ...
	 * @return boolean|mixed|Ambigous <string, boolean, mixed>
	 */
	public function deviceCompelBind($data)
	{
		if (!$this->access_token && !$this->checkAuth())
			return false;
		$data = array(
		...
		$result = $this->http_post(self::API_BASE_URL_PREFIX . self::DEVICE_COMPELBIND . 'access_token=' . $this->access_token, self::json_encode($data));
		$this->log($result);
		if ($result) {
			$json = json_decode($result, true);
	 ...
	 * @return boolean array
	 */
	public function DeviceVerifyQrcode($ticket)
	{
		if (!$this->access_token && !$this->checkAuth())
			return false;
		$data = array(
	 ...
	 * @return boolean|mixed|Ambigous <boolean, mixed>
	 */
	public function getDeviceStat($deviceid)
	{
		if (!$this->access_token && !$this->checkAuth())
			return false;
		$data = array(
		...
		$result = $this->http_get(self::API_BASE_URL_PREFIX . self::DEVICE_GET_STAT . 'access_token=' . $this->access_token . '&device_id=' . $deviceid);
		$this->log($result);
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
	 ...
	 * 发送消息给设备
	 * @param array $data
	 * @return boolean JSON "ozhqUt5rQoWDTGBuHQmB6Jp4j7VU","content": "fDIBAzYwfQ=="}';
	 */
	...
	public function getOpenId($data)
	{
		if (!$this->access_token && !$this->checkAuth())
			return false;
		$result = $this->http_get(self::API_BASE_URL_PREFIX . self::DEVICE_GET_OPENID . 'access_token=' . $this->access_token . '&device_type=' . $data['device_type'] . '&device_id=' . $data['device_id']);
		$this->log($result);
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
	 ...
	 */
	public function getBindDevice($data)
	{
		if (!$this->access_token && !$this->checkAuth())
			return false;
		$result = $this->http_get(self::API_BASE_URL_PREFIX . self::DEVICE_GET_USER_BIND_DEVICE . 'access_token=' . $this->access_token . '&openid=' . $data['openid']);
		$this->log($result);
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
	...
	public function replyDeviceMsg($devicemsg)
	{
		$msg = array(
			'ToUserName' => $devicemsg['FromUserName'],
			'FromUserName' => $devicemsg['ToUserName'],
			'MsgType' => $devicemsg['MsgType'],
			'CreateTime' => $devicemsg['CreateTime'],
			'DeviceType' => $devicemsg['DeviceType'],
			'DeviceID' => $devicemsg['DeviceID'],
			'SessionID' => $devicemsg['SessionID'],
			'Content' => $devicemsg['Content'],
		);
		$this->Message($msg);
		return $this;
	}
	...
	{
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_get(self::API_URL_PREFIX . self::USER_TAG_GET . 'access_token=' . $this->access_token);
		// var_dump($result);
	...
	public function createUserTag($name)
	{
		...
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::USER_TAG_CREATE . 'access_token=' . $this->access_token, self::json_encode($data));
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function updateUserTag($tagid, $name)
	{
		$data = array(
			'tag' => array('id' => $tagid, 'name' => $name),
		);
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::USER_TAG_EDIT . 'access_token=' . $this->access_token, self::json_encode($data));
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function delUserTag($tagid)
	{
		$data = array(
			'tag' => array('id' => $tagid),
		);
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::USER_TAG_DEL . 'access_token=' . $this->access_token, self::json_encode($data));
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function getUserTagFans($tagid)
	{
		$data = array(
			'tagid' => $tagid,
			'next_openid' => ''
		...
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::USER_TAG_FANS_GET . 'access_token=' . $this->access_token, self::json_encode($data));
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function getMemberTag($openid, $tagid)
	{
		$data = array(
			'openid' => $openid,
		);
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::MEMBER_TAG_DEL . 'access_token=' . $this->access_token, self::json_encode($data));
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function setMemberTag($tagid, $openid)
	{
		$data = array(
		...
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::MEMBER_TAG_SET . 'access_token=' . $this->access_token, self::json_encode($data));
		file_put_contents('./textkey.log', "\n" . '[' . date('Y-m-d H:i:s') . '] wechat ' . var_export($result, true), FILE_APPEND);
		// var_dump($result);
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
	...
	public function delMemberTag($tagid, $openid)
	{
		$data = array(
			'openid_list' => array($openid),
			'tagid' => $tagid
	...
	public function createTagMenu($menu)
	{

		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_URL_PREFIX . self::CREATE_TAG_MENU . 'access_token=' . $this->access_token, self::json_encode($menu));
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function uploadImg($img)
	{
		...
		if (!$this->access_token && !$this->checkAuth()) return false;
		if (!$this->isImage($img)) return false;
		$img = json_decode(array('media', '@' . $img));
		$result = $this->http_post(self::API_URL_PREFIX . self::MEDIA_UPLOADIMG_URL . 'access_token=' . $this->access_token, $img, true);
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
